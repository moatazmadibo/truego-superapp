-- =========================================================
-- TrueGo Driver Vehicle Type Editable Profile
-- =========================================================
...
declare
  result jsonb;
  clean_vehicle_type text;
begin
  clean_vehicle_type := nullif(trim(coalesce(p_vehicle_type, '')), '');

  if clean_vehicle_type is not null and clean_vehicle_type not in ('car', 'motorcycle') then
    raise exception 'Invalid vehicle type: %', clean_vehicle_type;
  end if;

  update public.demo_drivers
  set
    vehicle_type = coalesce(clean_vehicle_type, vehicle_type),
...
grant execute on function public.update_demo_driver_vehicle_profile(
  text, text, text, text, text, text, integer, date, date, text
