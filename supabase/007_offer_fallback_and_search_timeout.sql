...
alter table public.rides
add column if not exists offer_attempt_count integer not null default 0;

alter table public.rides
add column if not exists attempted_demo_driver_ids jsonb not null default '[]'::jsonb;
...
drop function if exists public.advance_demo_ride_offer(uuid, integer, integer);
drop function if exists public.dispatch_ride_to_nearest_demo_driver(uuid, integer);
drop function if exists public.sync_demo_ride_offer_state(uuid);

create function public.advance_demo_ride_offer(
  p_ride_id uuid,
...
begin
  ...
  if v_ride.status in ('driver_arriving', 'in_progress', 'completed', 'cancelled', 'no_driver_available') then
    return v_ride;
  end if;
...
create function public.sync_demo_ride_offer_state(
  p_ride_id uuid
)
returns public.rides
language plpgsql
